...
export function initPersonalizeModal(sp500Companies, usStockSymbols = []) {
  ...
  function renderRangeViz(bestData, worstData) {
    ...
    const d3 = window.d3;
    if (!d3) { console.warn('renderRangeViz: d3 not found'); return; }
    ...
    const yScale  = d3.scaleLinear()
      .domain([Math.min(...allVals) * 0.95, Math.max(...allVals) * 1.05]).nice().range([H, 0]);

    g.append('g').attr('class', 'prr-grid')
    ...
    const area = d3.area()
      .x((d) => xScale(d.date)).y0((d) => yScale(d.worst)).y1((d) => yScale(d.best)).curve(d3.curveMonotoneX);
    g.append('path').datum(pts).attr('class', 'prr-range-band').attr('d', area);

    const lineFn = d3.line().curve(d3.curveMonotoneX);
    g.append('path').datum(pts).attr('class', 'prr-line-worst')
      .attr('d', lineFn.x((d) => xScale(d.date)).y((d) => yScale(d.worst)));
    g.append('path').datum(pts).attr('class', 'prr-line-best')
    ...
    svg.append('rect')
      .attr('x', margin.left).attr('y', margin.top).attr('width', W).attr('height', H).attr('fill', 'transparent')
  ...
  return {
    async loadAndRender(data, options = {}) {
      ...
      if (isOwnMode) {
        ...
        const _rr1 = document.getElementById('personalize-range-viz'); if (_rr1) _rr1.hidden = true;
        renderOwnPicksViz(bestVizEl, bestData);
      } else if (isStrategyMode) {
        if (worstSlide) worstSlide.hidden = true;
        const _rr2 = document.getElementById('personalize-range-viz'); if (_rr2) _rr2.hidden = true;
